...
interface Props {
  data: Topic[]
  onTopicClick?: (id: string, event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => void;
  onTopicMoreClick?: (id: string, event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => void;
    ...
    <div className="flex flex-col text-[#0F1419] mb-4">
      ...
      {data.map(item => (
        <HotTopicItem
          key={item.id}
          data={item}
          onTopicClick={(event) => onTopicClick ? onTopicClick(item.id, event) : null}
          onMoreClick={(event) => onTopicMoreClick ? onTopicMoreClick(item.id, event) : null}
        />
      ))}
      <a
